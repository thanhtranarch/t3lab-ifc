...
let clashPropertyCacheA={}, clashPropertyCacheB={}; // eid→{propName:value}
...
window.applyClashPreset=function(presetKey){
  ...
  ['A','B'].forEach(side=>{
    ...
    let categories; // final list of Revit category labels for this side
    ...
    clashRuleRows[side]=categories.map(t=>({
      elementType:t, property:'None', operator:'', value:'', action:'Add'
    }));
    renderClashRules(side);
  });
...
function getSelectedCats(side){return [...resolveClashElementTypes(side)]}
...
window.toggleClashMode=function(){
  if(compareResult){log('Exit compare first');return}
  clashMode=!clashMode;
...
window.exitClashMode=function(){
  ...
  clashSubsets.forEach(s=>{if(s.parent)s.parent.remove(s)});
  clashSubsets=[];clashResults=[];currentClashIdx=-1;
  clashPropertyCacheA={};clashPropertyCacheB={};
  // Remove focus highlights
  const oldFocus=[];
  scene.traverse(c=>{if(c.userData?.clashFocus)oldFocus.push(c)});
  ...
  for(let i=0;i<2;i++){
    ...
    loadedModels[i].traverse(c=>{if(c.isMesh){
      if(c.userData._clashOrigMats){c.material=c.userData._clashOrigMats;delete c.userData._clashOrigMats}
      c.visible=true;
    }});
  }
...
window.runClashDetection=async function(){
  ...
  lt.textContent=`Done! ${clashResults.length} clashes`;lf.style.width='100%';
...
function showClashResults(){
  ...
  if(clashResults.length>0){document.getElementById('btnExportClashCSV').style.display='';document.getElementById('btnExportClashBCF').style.display=''}
  ...
  for(let i=0;i<2;i++){
    ...
    loadedModels[i].traverse(c=>{if(c.isMesh){
      ...
      ms.forEach(m=>{
        m.transparent=true;m.opacity=0.55;m.depthWrite=true;m.needsUpdate=true;
...
window.focusClash=function(idx){
  ...
  const matFocusA=new THREE.MeshPhongMaterial({color:0xef4444,transparent:true,opacity:0.7,side:THREE.DoubleSide,depthWrite:true,clippingPlanes:clipPlanes});
  const matFocusB=new THREE.MeshPhongMaterial({color:0x3b82f6,transparent:true,opacity:0.7,side:THREE.DoubleSide,depthWrite:true,clippingPlanes:clipPlanes});
  ...
  [
    ...
    try{
      ...
      if(sub){
        ...
        sub.traverse(ch=>{if(ch.isMesh)ch.userData.clashFocus=true});
      }
    }catch(e){}
  });
  ...
  if(clashSubsets[0]&&clashSubsets[0].name==='clashMarkers'){
    clashSubsets[0].children.forEach(ch=>{
      if(ch.userData.clashIdx===idx&&ch.material&&!ch.material.wireframe){
        ch.material.opacity=0.95;ch.material.needsUpdate=true;
      }else if(ch.material&&!ch.material.wireframe&&ch.userData.clashIdx!==undefined){
        ch.material.opacity=0.35;ch.material.needsUpdate=true;
      }
    });
  }
...
window.exportClashBCF=async function(){
  if(!clashResults.length){log('No clashes to export');return}
  if(!window.JSZip){
  ...
  const saveSlider={
    Xp:document.getElementById('slXp')?.value, Xn:document.getElementById('slXn')?.value,
    Yp:document.getElementById('slYp')?.value, Yn:document.getElementById('slYn')?.value,
    Zp:document.getElementById('slZp')?.value, Zn:document.getElementById('slZn')?.value,
  };
  ...
  for(let i=0;i<clashResults.length;i++){
    ...
    }catch(e){log('Clash snapshot err #'+(i+1)+':',e?.message)}
  ...
  const focusHL=[];
  scene.traverse(c=>{if(c.userData?.clashFocus)focusHL.push(c)});
  ...
  if(!saveSectionActive && sectionActive){
    ...
    if(sb){scene.remove(sb)}
    // Reset clipPlanes to no-clip
    if(clipPlanes && clipPlanes.length===6){
      clipPlanes[0].constant=99999;clipPlanes[1].constant=99999;
      clipPlanes[2].constant=99999;clipPlanes[3].constant=99999;
      clipPlanes[4].constant=99999;clipPlanes[5].constant=99999;
    }
  }else if(saveSectionActive){
    // User had section box — restore its slider values exactly.
    const setSl=(id,v)=>{const el=document.getElementById(id);if(el&&v!==undefined)el.value=v};
  ...
  if(savePrevClashIdx>=0 && savePrevClashIdx<clashResults.length){
    ...
    camera.position.copy(saveCam);controls.target.copy(saveTgt);controls.update();
  }
  renderer.render(scene,camera);

  const blob=await zip.generateAsync({type:'blob'});
  const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='ifc-delta-clashes.bcf';a.click();
  setStatus('done','BCF exported');setTimeout(()=>setStatus('',''),3000);
