...
let _searchCache = null;     // {elements[], propNames[], typeNames[], key}
let _searchResults = [];     // filtered subset
...
async function searchBuildCache(){
  ...
  for(let mi=0; mi<loadedModels.length; mi++){
    ...
    for(const gid in props){
      ...
      const propMap = new Map(); // propName → {value, psetName}
...
window.searchSelect = async function(idx){
  ...
  try{
    ...
    if(sub){sub.position.copy(loadedModels[e.modelIdx].position); sub.updateMatrixWorld(true); window._lastHL={subset:sub,mid:modelID};}
  }catch(err){}
...
window.searchIsolateAll = function(){
  ...
  for(const [mi, eids] of byModel){
    ...
    try{
      const mat = new THREE.MeshPhongMaterial({color:0x22c55e,transparent:true,opacity:0.8,side:THREE.DoubleSide,depthTest:true,clippingPlanes:clipPlanes});
      ...
      if(sub){sub.position.copy(loadedModels[mi].position); sub.updateMatrixWorld(true);}
    }catch(e){}
  }
